...
const ThreeDTrackingHUD = ({
  ...
  const getQualityIcon = (quality) => {
    if (quality >= 0.8) return <CheckCircledIcon className="text-green-400" />;
    if (quality >= 0.6) return <ExclamationTriangleIcon className="text-yellow-400" />;
    return <CrossCircledIcon className="text-red-400" />;
  };
        ...
        >
          <div className="bg-gray-800/90 backdrop-blur-sm border border-gray-600 rounded-lg p-2 cursor-pointer">
            <div className="flex items-center gap-2">
              <span className="text-lg">{is3DMode ? <TargetIcon /> : <HandIcon />}</span>
        ...
        <div className="bg-gray-800/90 backdrop-blur-sm border border-gray-600 rounded-lg p-4 min-w-64">
          ...
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold text-white flex items-center gap-2">
              {is3DMode ? <><TargetIcon /> 3D Tracking</> : <><HandIcon /> 2D Tracking</>}
            </h3>
            <div className="flex items-center gap-2">
              <button
              ...
              >
                {showDetails ? <ReaderIcon /> : <ReaderIcon className="opacity-50" />}
              </button>
              <button
                className="text-xs text-gray-400 hover:text-white transition-colors"
                onClick={onToggleMinimize}
          ...
          <div className="space-y-2 mb-4">
            ...
            {is3DMode && (
              <div className="flex justify-between items-center">
                ...
                <span className={`text-xs ${isCalibrated ? 'text-green-400' : 'text-orange-400'}`}>
                  {isCalibrated ? <CheckCircledIcon /> : <ExclamationTriangleIcon />}
                </span>
              </div>
            )}
            ...
            <div className="flex justify-between items-center">
              ...
              <span className={`text-xs ${handState?.isTracking ? 'text-green-400' : 'text-red-400'}`}>
                {handState?.isTracking ? <CheckCircledIcon /> : <CrossCircledIcon />}
              </span>
            </div>
          </div>
          ...
          {is3DMode && !isCalibrated && (
            <motion.div
              className="mt-3 p-2 bg-orange-500/20 border border-orange-500/30 rounded text-xs text-orange-400 flex items-center gap-2"
              animate={{ opacity: [0.7, 1, 0.7] }}
              transition={{ repeat: Infinity, duration: 2 }}
            >
              <ExclamationTriangleIcon /> 3D mode requires calibration for accurate tracking
            </motion.div>
          )}

          {is3DMode && performance.qualityScore < 0.4 && (
            <motion.div
              className="mt-3 p-2 bg-red-500/20 border border-red-500/30 rounded text-xs text-red-400 flex items-center gap-2"
              animate={{ opacity: [0.7, 1, 0.7] }}
              transition={{ repeat: Infinity, duration: 1.5 }}
            >
              <CrossCircledIcon /> Poor tracking quality - check lighting and hand visibility
            </motion.div>
          )}
        </div>
      </motion.div>
